...
final class HotkeyServiceCompatibilityTests: XCTestCase {
    ...
    @MainActor
    func testAccessibilityTrustRefreshUpdatesPublishedState() {
        let service = HotkeyService()
        var isTrusted = false
        service.accessibilityTrustedProvider = { isTrusted }

        service.resumeMonitoring()
        XCTAssertFalse(service.isAccessibilityTrustedForGlobalHotkeys)

        isTrusted = true
        service.refreshMonitorForCurrentAccessibilityTrust()
        XCTAssertTrue(service.isAccessibilityTrustedForGlobalHotkeys)
    ...
    @MainActor
    func testKeyWithModifiersPushToTalkDoesNotStopWhenPhysicalKeysRemainDown() throws {
        let service = HotkeyService()
        service.suspendMonitoring()

        service.setHotkeyForTesting(commandOptionAHotkey(), for: .pushToTalk)
        var physicallyDown: Set<UInt16> = [0x00, 0x37, 0x3A]
        service.setPhysicalKeyStateProviderForTesting { physicallyDown.contains($0) }
        ...
        let transientModifierLoss = try makeFlagsChangedEvent(keyCode: 0x3A, modifierFlags: [.command])
        ...
        XCTAssertTrue(service.processEventForTesting(transientModifierLoss, source: .monitor))
        XCTAssertEqual(stopCount, 0)

        physicallyDown = [0x37, 0x3A]
        XCTAssertTrue(service.processEventForTesting(keyUp, source: .monitor))
        XCTAssertEqual(stopCount, 1)
    }
    ...
    @MainActor
    func testPushToTalkIgnoresExtraKeysUntilShortcutRelease() throws {
        let service = HotkeyService()
        service.suspendMonitoring()

        service.setHotkeyForTesting(commandOptionComboHotkey(), for: .pushToTalk)
        var physicallyDown: Set<UInt16> = [0x37, 0x3A]
        service.setPhysicalKeyStateProviderForTesting { physicallyDown.contains($0) }
